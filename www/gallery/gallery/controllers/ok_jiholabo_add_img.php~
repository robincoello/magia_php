...
//
$name = (!empty($_REQUEST['name'])) ? clean($_REQUEST['name']) : false;
$title = (!empty($_REQUEST['title'])) ? clean($_REQUEST['title']) : false;
$description = (!empty($_REQUEST['description'])) ? clean($_REQUEST['description']) : false;
$alt = (!empty($_REQUEST['alt'])) ? clean($_REQUEST['alt']) : false;
$url = (!empty($_REQUEST['url'])) ? clean($_REQUEST['url']) : false;
$file = (!empty($_FILES['file']) ) ? $_FILES['file'] : false;
$redi = (!empty($_REQUEST['redi'])) ? clean($_REQUEST['redi']) : false;
//
$side = ($_REQUEST["side"] != "") ? clean($_REQUEST["side"]) : false;
$item = ($_REQUEST["item"] != "") ? clean($_REQUEST["item"]) : false;
$item_id = ($_REQUEST["item_id"] != "") ? clean($_REQUEST["item_id"]) : false;

$picture_id = false;
// si halla q hay ya una principal, manda cero 
//$principal = (products_pictures_search_principal_by_product_id($item_id)) ? 0 : 1;
$principal = 0;
$order_by = 1;
$status = 1;
$code = magia_uniqid();
...
// solo para control
$acepted_ext = $sendfile->_get_ext_acepted();

// donde se debe enviar el archivo? 
// con el / al final 
$sendfile->_set_path("www/gallery/img/jiho_audio/"); // debe terminar en /
// verificar
$path = $sendfile->_get_path();

// renombramos el archivo con el nombre de la emprea y un damdom 
// 
//$newFileName = _options_option('config_subdomain') . magia_uniqid();
$newFileName = $item . "_" . $side . "_" . $item_id;
//
