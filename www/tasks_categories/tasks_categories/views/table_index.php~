            ...
            foreach ($tasks_categories as $tasks_categories_item) {
                $menu = '<div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                              <li><a href="index.php?c=tasks_categories&a=details&id=' . $tasks_categories_item["id"] . '">' . _tr("Details") . '</a></li>
                              <li><a href="index.php?c=tasks_categories&a=edit&id=' . $tasks_categories_item["id"] . '">' . _tr("Edit") . '</a></li>
                              <li role="separator" class="divider"></li>
                              <li><a href="index.php?c=tasks_categories&a=delete&id=' . $tasks_categories_item["id"] . '">' . _tr("Delete") . '</a></li>
                            </ul>
                          </div>';
                echo "<tr id=\"$tasks_categories_item[id]\">";
                $checked_array = json_decode(_options_option("config_tasks_categories_show_col_from_table"), true);
                foreach (tasks_categories_db_col_list_from_table($c) as $col_name) {
                    if (isset($checked_array[$col_name]) || !isset($checked_array)) {
                        //echo "<td>$tasks_categories_item[$col_name]</td>";
                        echo "<td>" . tasks_categories_add_filter($col_name, $tasks_categories_item[$col_name]) . "</td>";
                    }
                } echo "<td>$menu</td>";
                echo "</tr>";
            }
