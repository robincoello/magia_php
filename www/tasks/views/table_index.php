<table class="table table-striped">
        ...
        <tr class="info">
            ...
            foreach (tasks_db_col_list_from_table($c) as $th) {
                ...
                if (isset($checked_array[$th]) || !isset($checked_array)) {
                    ...
                    echo '<th><a href="index.php?c=tasks&order_col=' . $th . '' . $link_order_way . ' ">' . _tr(ucfirst($th)) . ' ' . $order_icon_show . '</a></th>';
                }
                $order_icon_show = false;
            }
            ?>
        ...
        <tr>
            ...
            foreach ($tasks as $tasks_item) {
                $menu = '<div class="dropdown">
                            ...
                            <?php if (permissions_has_permission($u_rol, "tasks", "read")) { ?>
                              <li><a href="index.php?c=tasks&a=details&id=' . $tasks_item["id"] . '">' . _tr("Details") . '</a></li>
                            <?php } ?>
                            
                            <?php if (permissions_has_permission($u_rol, "tasks", "update")) { ?>
                                <li><a href="index.php?c=tasks&a=edit&id=' . $tasks_item["id"] . '">' . _tr("Edit") . '</a></li>
                            <?php } ?>    

                             <?php if (permissions_has_permission($u_rol, "tasks", "delete")) { ?>
                              <li role="separator" class="divider"></li>                              
                              <li><a href="index.php?c=tasks&a=delete&id=' . $tasks_item["id"] . '">' . _tr("Delete") . '</a></li>
                            <?php } ?>    
                            

                            </ul>
                          </div>';
                echo "<tr id=\"$tasks_item[id]\">";
                $checked_array = json_decode(_options_option("config_tasks_show_col_from_table"), true);
                foreach (tasks_db_col_list_from_table($c) as $col_name) {
                    if (isset($checked_array[$col_name]) || !isset($checked_array)) {
                        //echo "<td>$tasks_item[$col_name]</td>";
                        // si una columna se llama xx
                        if ($col_name == "xxxxxxxxxxxxxx") { // se pone un filtro en este caso la primera mayuscula
                            echo "<td>" . tasks_add_filter($col_name, $tasks_item[$col_name], ucfirst($col_name)) . "</td>";
                        } else {
                            echo "<td>" . tasks_add_filter($col_name, $tasks_item[$col_name]) . "</td>";
                        }
                    }
                } echo "<td>$menu</td>";
                echo "</tr>";
    ...
    <?php include view("tasks", "table_index_form_add"); ?>
