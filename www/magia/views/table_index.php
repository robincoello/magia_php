        ...
        <tr class="info">
            ...
            foreach (magia_db_col_list_from_table($c) as $th) {
                ...
                if (isset($checked_array[$th]) || !isset($checked_array)) {
                    ...
                    echo '<th><a href="index.php?c=magia&order_col=' . $th . '' . $link_order_way . ' ">' . _tr(ucfirst($th)) . ' ' . $order_icon_show . '</a></th>';
                }
                $order_icon_show = false;
            }
            ?>
            ...
            foreach ($magia as $magia_item) {
                $menu = '<div class="dropdown">
                            ...
                            <ul class="dropdown-menu" aria-labelledby="dropdownMenu1">
                              <li><a href="index.php?c=magia&a=details&id=' . $magia_item["id"] . '">' . _tr("Details") . '</a></li>
                              <li><a href="index.php?c=magia&a=edit&id=' . $magia_item["id"] . '">' . _tr("Edit") . '</a></li>
                              <li role="separator" class="divider"></li>
                              <li><a href="index.php?c=magia&a=delete&id=' . $magia_item["id"] . '">' . _tr("Delete") . '</a></li>
                            </ul>
                          </div>';
                echo "<tr id=\"$magia_item[id]\">";
                $checked_array = json_decode(_options_option("config_magia_show_col_from_table"), true);
                foreach (magia_db_col_list_from_table($c) as $col_name) {
                    if (isset($checked_array[$col_name]) || !isset($checked_array)) {
                        //echo "<td>$magia_item[$col_name]</td>";
                        echo "<td>" . magia_add_filter($col_name, $magia_item[$col_name]) . "</td>";
                    }
                } echo "<td>$menu</td>";
                echo "</tr>";
            }
