...
switch ($w) {
    ...
    case "actived":
        ...
        // puede hacer falta
        $url = "index.php?c=modules&a=search&w=actived&status=" . $status;
        $pagination->setUrl($url);
    ...
    case "inactived":
        $status = (isset($_GET["status"]) && $_GET["status"] != "" ) ? clean($_GET["status"]) : 0;
        ################################################################################
        $pagination = new Pagination($page, modules_list_without_father_by_status(0));
        // puede hacer falta
        $url = "index.php?c=modules&a=search&w=inactived&status=" . $status;
        $pagination->setUrl($url);
        $modules = modules_list_without_father_by_status(0, $pagination->getStart(), $pagination->getLimit());
        ################################################################################ 
        break;
