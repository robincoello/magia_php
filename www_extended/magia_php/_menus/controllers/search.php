...
switch ($w) {
    ...
    case "by_location_father_id":
        ...
        ################################################################################
        $pagination = new Pagination($page, _menus_search_by_location_father_id($location, $father_id));
        // puede hacer falta
        $url = "index.php?c=_menus&a=search&w=by_father_id&father_id=" . $father_id;
        $pagination->setUrl($url);
        $_menus = _menus_search_by_location_father_id($location, $father_id, $pagination->getStart(), $pagination->getLimit());
        ################################################################################ 
        break;

    case "by_c_and_l": // controller and location
        $controller = (isset($_GET["controller"]) && $_GET["controller"] != "" ) ? clean($_GET["controller"]) : false;
        $location = (isset($_GET["location"]) && $_GET["location"] != "" ) ? clean($_GET["location"]) : false;
        ################################################################################
        $pagination = new Pagination($page, _menus_search_by_controller_location($controller, $location));
        // puede hacer falta
        $url = "index.php?c=_menus&a=search&w=by_c_and_l&controller=$controller&location=$location";
        $pagination->setUrl($url);
        $_menus = _menus_search_by_controller_location($controller, $location, $pagination->getStart(), $pagination->getLimit());
        ################################################################################ 
        break;

    case "full": // controller and location
        $location = (isset($_GET["location"]) && $_GET["location"] != "" ) ? clean($_GET["location"]) : false;
        $father_id = (isset($_GET["father_id"]) && $_GET["father_id"] != "" ) ? clean($_GET["father_id"]) : false;
        $label = (isset($_GET["label"]) && $_GET["label"] != "" ) ? clean($_GET["label"]) : false;
        $controller = (isset($_GET["controller"]) && $_GET["controller"] != "" ) ? clean($_GET["controller"]) : false;
//        $url = (isset($_GET["url"]) && $_GET["url"] != "" ) ? clean($_GET["url"]) : false;
        ...
        ################################################################################
        $pagination = new Pagination($page, _menus_search_full($location, $father_id, $label, $controller, $target, $icon, $order_by, $status));
        // puede hacer falta
        $url = "index.php?c=_menus&a=search&w=full&location=$location&father_id=$father_id&label=$label&controller=$controller&target=$target&icon=$icon&order_by=$order_by&status=$status";
        $pagination->setUrl($url);
        $_menus = _menus_search_full($location, $father_id, $label, $controller, $target, $icon, $order_by, $status, $pagination->getStart(), $pagination->getLimit());
        ################################################################################ 
        break;
